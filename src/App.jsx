...
function RiddleGame({onBack,onScore}){
  ...
  const next=()=>{if(pendingStars>0){onScore(pendingStars);setPendingStars(0)}let n;do{n=Math.floor(Math.random()*RIDDLES.length)}while(n===idx&&RIDDLES.length>1);setIdx(n);setShowAnswer(false);setShowHint(false)};
  const reveal=()=>{if(!showAnswer){setShowAnswer(true);if(!tried.has(idx)){setTried(p=>new Set([...p,idx]));setSolved(s=>s+1);setPendingStars(3)}}};
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
      <button onClick={()=>{if(pendingStars>0)onScore(pendingStars);onBack()}} style={S.backBtn}>← Back</button>
